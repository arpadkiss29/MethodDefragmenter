...
public abstract class AbstractInternalCodeFragment {
	...
	public static List<AbstractInternalCodeFragment> allNodesLeafs = new ArrayList<AbstractInternalCodeFragment>();
	protected List<AbstractInternalCodeFragment> children = null;
	...
	public boolean isEmptyNode() {
		if (children.size() == 0 && internalASTNodes.size() == 0) {
			if (this instanceof InternalCodeFragmentLeaf) {
				if (((InternalCodeFragmentLeaf) this).getStatementsLength() == 0) {
					return true;
				}
			} else {
				return true;
	...
	private double calculateMetric(List<InternalCodeFragmentLeaf> fragments) {
	...
	private int calculateFirstLine() {
		...
		if (firstChild instanceof InternalCodeFragmentLeaf) {
			return ((InternalCodeFragmentLeaf) firstChild).getFragmentFirstLine();
		} else {
	...
	private int calculateLastLine() {
		...
		if (lastChild instanceof InternalCodeFragmentLeaf) {
			return ((InternalCodeFragmentLeaf) lastChild).getFragmentLastLine();
		} else {
	...
	public List<AbstractInternalCodeFragment> identifyFunctionalSegments() {
		List<AbstractInternalCodeFragment> receivedNodes = new ArrayList<AbstractInternalCodeFragment>();
		for (int i = 0; i < children.size(); i++) {
		...
		for (int i = 0; i < nodes.size() - 1; i++) {
			for (int j = i + 1; j < nodes.size(); j++) {
				...
				if (nodes.get(i) instanceof InternalCodeFragmentLeaf) {
					...
					for (AbstractInternalCodeFragment eachNode : nodes.get(i).children) {
				...
				if (nodes.get(j) instanceof InternalCodeFragmentLeaf) {
					...
					for (AbstractInternalCodeFragment eachNode : nodes.get(j).children) {
						if (eachNode instanceof InternalCodeFragmentLeaf)
							fragmentsCombination.add((InternalCodeFragmentLeaf) eachNode);
					}
				}
	...
	public List<AbstractInternalCodeFragment> getAllEnviousNodes() {
		...
		for (AbstractInternalCodeFragment node : children) {
			if (node instanceof InternalCodeFragmentLeaf && ((InternalCodeFragmentLeaf) node).isEnvy()) {
				nodes.add(node);
			} else {
				if (node.isEnvy()) {
					nodes.add(node);
		...
		if (functionalSegmentNodes.contains(this) && (possiblyRelatedFlag != true)) {
			try {
				if (colorCounter < 17) {
					...
					for (int i = 0; i < children.size(); i++) {
						if (children.get(i) instanceof InternalCodeFragmentLeaf) {
							int start = ((InternalCodeFragmentLeaf) children.get(i)).getFragmentFirstLine();
							int end = ((InternalCodeFragmentLeaf) children.get(i)).getFragmentLastLine();
					...
					for (int i = 0; i < leafsReceived.size(); i++) {
						int start = ((InternalCodeFragmentLeaf) leafsReceived.get(i)).getFragmentFirstLine();
						int end = ((InternalCodeFragmentLeaf) leafsReceived.get(i)).getFragmentLastLine();
						Position fragmentPosition = new Position(start, (end - start));
						IMarker mymarker = SelectionView.createMarker(file, fragmentPosition);
						SelectionView.addAnnotation(mymarker, textEditor, "annotationColor_" + colorCounter,
								fragmentPosition);
					...
					for (int i = 0; i < cohesivlyRelatedNodes.size(); i++) {
						if (cohesivlyRelatedNodes.get(i) instanceof InternalCodeFragmentLeaf) {
							int start = ((InternalCodeFragmentLeaf) cohesivlyRelatedNodes.get(i)).getFragmentFirstLine();
							int end = ((InternalCodeFragmentLeaf) cohesivlyRelatedNodes.get(i)).getFragmentLastLine();
							Position fragmentPosition = new Position(start, (end - start));
							IMarker mymarker = SelectionView.createMarker(file, fragmentPosition);
							SelectionView.addAnnotation(mymarker, textEditor, "annotationColor_" + colorCounter,
									fragmentPosition);
	...
	private HashMap<String, ITypeAccesses> accessClassesMapping = new HashMap<String, ITypeAccesses>();
	...
	public HashMap<String, Integer> getFdp(String analyzedClass, boolean staticFields, Integer minBlockSize, boolean libraryCheck) {
		...
		return storedFDP;//not sure if ok
	}

	public boolean verifyFeatureEnvy(int ATFDTreshold, int FDPTreshold, double LAATreshold, String analyzedClass, boolean staticFields,
		...
		if (local) {
			...
			for (ITypeAccesses iTypeAccesses : accessClassesMapping.values()) {
				accessForeignData += iTypeAccesses.getNumberOfAccesses();
			}
			int totalAccesses = accessForeignData + localAttrAccess;
			foreignDataProviders = accessClassesMapping.size();

			if (accessForeignData > ATFDTreshold
					&& (localAttrAccess > 0 ? (localAttrAccess * 1.0) / totalAccesses : 0) < (LAATreshold)
			...
			for (int i = 0; i < children.size(); i++) {
				if (children.get(i).verifyFeatureEnvy(ATFDTreshold, FDPTreshold, LAATreshold, analyzedClass, staticFields,
	...
	public void colorEnvyLeafNodes(ITextEditor textEditor, IFile file) throws CoreException {
		if (isEnvy && this instanceof InternalCodeFragmentLeaf) {
	...
	public HashMap<String, ITypeAccesses> getAccessClassesMapping() {
